...
| **Chữ ký tay → ink annotation** | ✅ *(đã sửa + đã có test canh)* | code map toạ độ sang **annotation-local space** (không phải page-space) → `/InkList` nằm trong `/Rect` + MuPDF render ra. Nay ghim bởi `SignaturePointTests` (+ mutation check chứng minh test bắt được hồi quy page-space cũ) |
| Ghi chú | ✅ | `⇧⌘N` |
...
### Đã hết bị chặn và đã sửa
- ✅ **C1 — engine trả `kind: "unknown"` cho highlight và ink.** Trước đây bị chặn vì nhánh này
  chưa có `Adapters/`. Sau khi WIP của Codex được commit và merge vào `main`, chỗ cần sửa đã có mặt.

  **Nguyên nhân:** `azpdf_annotations.js` → `kindForAnnotation()` chỉ map `Text`, `FreeText` và
  `Stamp`; mọi subtype khác rơi vào `"unknown"` — dù `PDFAnnotationKind` phía Swift **vốn đã khai
  báo sẵn** `highlight`, `ink`, `link`, `widget`, `redaction`. Chỉ thiếu mapping ở tầng JS.

  **Đã sửa:** thêm 5 nhánh map cho các subtype đó. Đo lại trên file thật:
  `unknown, unknown` → **`highlight`, `ink`** (free-text vẫn đúng như cũ).

  **Vì sao lọt lưới:** toàn bộ test trong `AZpdfMuPDFTests` **stub command runner**, nên đoạn
  JavaScript chứa lỗi chưa bao giờ được chạy trong test. Đã thêm
  `MuPDFAnnotationKindTests` chạy **mutool thật** trên fixture `annotated-highlight-ink.pdf`.
  Test được kiểm chứng ngược: gỡ fix ra thì fail đúng chỗ, lắp lại thì pass.
...
## Cam kết README ngoài danh sách tính năng — đã kiểm bằng cách chạy thật

| Cam kết trong README | Kiểm thế nào | Kết quả |
|---|---|---|
| "AZpdf không tải PDF, nội dung, mật khẩu hoặc lịch sử tài liệu lên máy chủ" | `grep URLSession` trong `App/ Core/ Views/ Stores/ Services/ Models/` | **0 kết quả** |
| "CI kiểm tra source để chặn API client mạng trong app/core" | job có thật trong `.github/workflows/ci.yml`; chạy `script/audit_local_first.sh` | exit 0 — *"no network client API found"* |
| Portable core tách khỏi nền tảng | chạy `script/audit_portable_core.sh` | exit 0 — *"Core remains Foundation-only"* |
| "Plugin chỉ được phát hiện cục bộ tại `~/Library/Application Support/AZpdf/Plugins/`" | `PluginRegistry.swift:14` | đúng đường dẫn README nêu |

## Ba nền tảng — trạng thái đối chiếu README

README (dòng 3) nêu rõ mức độ cam kết từng nền tảng, và kết quả kiểm khớp đúng với mức đó:

| Nền tảng | README cam kết gì | Kiểm được gì |
|---|---|---|
| **macOS** | *"nền tảng phát hành đầu tiên"* | 25/25 tính năng ✅, 119 test, 0 failure |
| **Linux** | *"v2 đang ở giai đoạn alpha"* | kit chạy thật trên Ubuntu 24.04: **23 pass / 0 fail / 4 skip** |
| **Windows** | *"nằm trong lộ trình tiếp theo"* | kit chạy thật trong VM: **`THIEU git` + `THIEU swift`** |

Kết quả Windows **đúng với mức cam kết**: README không hứa Windows chạy được ở bản này, và VM
xác nhận chưa có toolchain. Bootstrap dừng sạch kèm hướng dẫn cài, không fail mờ ám.
Muốn build thật trên Windows cần: `git`, Swift for Windows, và Visual Studio Build Tools (C++).

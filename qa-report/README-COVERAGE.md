...
| Ký CMS/PKCS#7 (.p7s) + xác minh | 🟡 | sheet mở đúng, **dropdown nạp identity thật từ Keychain**, save panel ra đúng tên `*.pdf.p7s`; nhưng bước ký cuối bị **macOS SecurityAgent** chặn — chỉ bạn cấp quyền được. Xem E5 |
...
## E5. Ký `.p7s` — chặn ở cổng quyền của macOS, kèm một lỗi im lặng

Luồng chạy đúng tới bước cuối:
1. Menu → sheet "Ký số bằng certificate" mở, mô tả chính xác (`.p7s` tách rời, PDF gốc không bị sửa)
2. Dropdown **nạp identity thật từ Keychain** (`kSecClassIdentity` không lọc policy nên lấy được cả
   identity code-signing; máy này có 2, S/MIME thì 0)
3. Save panel mở, tên mặc định `saveas-test.pdf.p7s` đúng quy ước

Bước ký cuối gọi private key → **macOS bật SecurityAgent** hỏi quyền. Đây là hành vi bảo mật đúng
của hệ điều hành, chỉ chủ máy cấp được; tự động hoá không vượt qua (và không nên vượt qua).

⚠️ **Lỗi phụ phát hiện được:** khi prompt đó không được chấp thuận, AZpdf **không báo gì cả** —
không file, không thông báo lỗi, sheet tự đóng. Người dùng bấm "Xuất chữ ký .p7s" rồi tưởng đã ký
xong trong khi không có gì được tạo. Cùng loại hỏng-im-lặng với các bug đã sửa ở trên.
*Đề xuất:* bắt lỗi từ `CertificateSigningService.detachedSignature` và hiện thông báo khi người dùng
từ chối cấp quyền.

**Để hoàn tất mục này:** bạn chạy lại PDF → "Ký bằng certificate…" → "Xuất chữ ký .p7s" → Save,
rồi bấm **Allow** ở hộp thoại Keychain. Sau đó kiểm chứng độc lập bằng:

```bash
openssl pkcs7 -inform DER -in <file>.p7s -print_certs -noout
openssl smime -verify -binary -inform DER -in <file>.p7s \
  -content <file>.pdf -noverify -out /dev/null   # phải PASS
openssl smime -verify -binary -inform DER -in <file>.p7s \
  -content <file-khac>.pdf -noverify -out /dev/null   # phải FAIL
```
